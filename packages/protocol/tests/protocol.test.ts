...
function fixture(directory: string, name: string) {
	return readFileSync(new URL(`../fixtures/${directory}/${name}`, import.meta.url));
}
...
test("loads the authoritative print-job schema and shared fixtures", () => {
	for (const name of ["valid-text-feed.json", "valid-rule.json", "valid-cut.json"]) {
		assert.equal(parsePrintJob(jsonFixture("print-job-v1", name)).schema_version, "1");
	}
	for (const name of [
...
test("keeps schema validity separate from the 1024-byte MQTT limit", () => {
	...
	const overLimit = fixture("print-job-v1", "schema-valid-over-mqtt-limit.json");
	assert.equal(atLimit.byteLength, MQTT_JOB_MAX_BYTES);
	assert.equal(overLimit.byteLength, MQTT_JOB_MAX_BYTES + 1);
	assert.doesNotThrow(() => encodePrintJob(parsePrintJob(JSON.parse(atLimit.toString("utf8")))));
	assert.throws(
...
test("validates every terminal job-result fixture", () => {
	for (const name of [
		...
		assert.equal(parseJobResult(jsonFixture("job-result-v1", name)).kind, "job_result");
	}
});

test("rejects result fields that contradict the terminal status", () => {
	const delivered = jsonFixture("job-result-v1", "delivered.json") as Record<string, unknown>;
	assert.throws(() => parseJobResult({ ...delivered, error_code: "INTERNAL_ERROR" }));
	const rejected = jsonFixture("job-result-v1", "rejected-validation.json") as Record<
		string,
		unknown
	>;
