...
verify)
    ...
    ;;
probe)
    exec "${ssh[@]}" 'sudo systemd-run --wait --pipe --collect --quiet --property=User=paperbridge --property=Group=paperbridge --property=WorkingDirectory=/opt/paperbridge/current --property=Environment=HOME=/var/empty --property=Environment=MISE_DATA_DIR=/opt/paperbridge/current/.mise --property=EnvironmentFile=/etc/paperbridge/paperbridge.env --property=NoNewPrivileges=yes --property=CapabilityBoundingSet= --property=AmbientCapabilities= --property=PrivateTmp=yes --property=PrivateDevices=yes --property=ProtectHome=true --property=ProtectSystem=strict --property=ReadWritePaths=/opt/paperbridge/current/.mise --property=ProtectKernelTunables=yes --property=ProtectKernelModules=yes --property=ProtectControlGroups=yes --property=ProtectClock=yes --property=RestrictSUIDSGID=yes --property=LockPersonality=yes --property=RestrictAddressFamilies=AF_UNIX\ AF_INET\ AF_INET6 --property=SystemCallArchitectures=native /opt/paperbridge/current/deploy/exedev/run-node.sh apps/api/src/main.ts'
    ;;
restart)
