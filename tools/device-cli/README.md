...
paperbridge --port /dev/cu.usbmodem101 wifi status
paperbridge --port /dev/cu.usbmodem101 mqtt status
...
semantic job. `wifi status` and `mqtt status` are no-output diagnostics for the
Wi-Fi control plane and optional MQTT tracer; neither submits a job or contacts
the printer.
